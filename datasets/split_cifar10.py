"""Split-CIFAR-10 class-incremental benchmark.

Single-head, class-IL (``multihead=False``, global labels 0..9). Mirrors
``split_cifar100`` but with 5 tasks of 2 classes by default. This is the one real
(non-MNIST) benchmark for the task-free pivot; the task-free *stream* is built on
top of these per-task splits by ``datasets/streams.py``.
"""
...
def _load_cifar10(data_root: str | Path, download: bool = True):
    try:
        from torchvision.datasets import CIFAR10
    except Exception as e:  # pragma: no cover - depends on optional install
        raise RuntimeError("Split-CIFAR-10 requires torchvision.") from e
...
def _make_class_incremental_tasks(
    ...
    for tid, classes in enumerate(class_groups):
        def select(x, y):
            ...
            return x[mask], y[mask].astype(np.int64)  # keep GLOBAL labels (class-IL)
        ...
        tasks.append(
            Task(
                task_id=tid,
                name=f"cifar10_task{tid}_{classes[0]}to{classes[-1]}",
...
def build_rotated_cifar10(cfg: Dict[str, Any], data_root: str | Path = "data") -> ContinualBenchmark:
    """Rotated-CIFAR-10: domain-IL, single 10-way head, all classes every task,
    a fixed rotation per task. The CNN-friendly, high-interference mirror of
    Permuted-MNIST (the regime where the large gate effects appeared on MNIST)."""
    from .rotated_mnist import _angles, _rotate_images
    ...
    for tid, angle in enumerate(angles):
        trbx, trby = subsample(train_x, train_y, max_train, seed + tid)
        tebx, teby = subsample(test_x, test_y, max_test, seed + 10_000 + tid)
        trx, tex = _rotate_images(trbx, angle), _rotate_images(tebx, angle)
        if cfg.get("normalize", True):
            trx = _normalize_cifar((trx * 255.0).astype(np.float32))
            tex = _normalize_cifar((tex * 255.0).astype(np.float32))
        a, b, vax, vay = train_val_split(trx, trby, val_fraction, seed + tid)
        tasks.append(
            Task(
                task_id=tid,
                name=f"rotcifar{tid}_{angle:.0f}",
                train=to_tensor_dataset(a, b),
                val=to_tensor_dataset(vax, vay),
                test=to_tensor_dataset(tex, teby),
                global_classes=list(range(n_classes)),
                n_classes=n_classes,
            )
        )
    return ContinualBenchmark(
        name="rotated_cifar10",
        tasks=tasks,
...
def build_split_cifar10(cfg: Dict[str, Any], data_root: str | Path = "data") -> ContinualBenchmark:
    seed = int(cfg.get("seed", 0))
    ...
    # task-IL variant: per-task heads + local labels (mirrors Split-MNIST exactly,
    # so the label-noise inversion testbed is directly comparable). Default path
    # below is the original single-head class-IL benchmark.
    if cfg.get("task_il", False):
        tasks = make_binary_tasks(
            train_x, train_y, test_x, test_y,
            class_pairs=class_groups,
